 ...
 * @edit detect hover-revealed actionable controls.
 */
...
export default (
	...
	/**
	 * @edit safely normalize tagName/nodeName.
	 * Some DOM/SVG/custom nodes expose tagName as a non-string object; calling
	 * tagName.toLowerCase() directly can crash the whole observer.
	 */
	function getTagName(element) {
		...
		if (typeof tagName.baseVal === 'string') return tagName.baseVal.toLowerCase()
	 ...
	 */
		function isTextNodeVisible(textNode) {
			try {
				...
				// Special case: when viewportExpansion is -1, skip viewport filtering
				// but still respect CSS hiding and ancestor clipping.
				if (viewportExpansion === -1) {
					try {
						...
						return style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0' && isElementVisible(parentElement)
					}
				}
				...
				if (!hasVisibleRectAfterAncestorClipping(rects, parentElement)) return false
		...
		function getOverflowValue(style, axis) {
			...
			return axis === 'x' ? (style.overflowX || style.overflow || 'visible') : (style.overflowY || style.overflow || 'visible')
		}
		...
		function hasVisibleRectAfterAncestorClipping(rects, element) {
			let visibleRects = Array.from(rects || []).filter(rect => rect.width > 0 && rect.height > 0)
			...
			while (el && el !== document.documentElement) {
				...
				if (s) {
					...
					const clipsX = isClippingOverflow(getOverflowValue(s, 'x'))
					const clipsY = isClippingOverflow(getOverflowValue(s, 'y'))
					if (clipsX || clipsY) {
						const clip = getCachedBoundingRect(el)
						if (!clip || clip.width <= 0 || clip.height <= 0) return false

						visibleRects = visibleRects.filter(rect => {
							const xVisible = !clipsX || (rect.right > clip.left && rect.left < clip.right)
							const yVisible = !clipsY || (rect.bottom > clip.top && rect.top < clip.bottom)
		...
		function getNearestScrollableAncestor(element) {
			...
			while (cur && cur.nodeType === Node.ELEMENT_NODE && cur !== document.body) {
				const style = getCachedComputedStyle(cur)
				if (style) {
					const overflowX = style.overflowX
					const overflowY = style.overflowY
					const scrollableX = overflowX === 'auto' || overflowX === 'scroll'
					const scrollableY = overflowY === 'auto' || overflowY === 'scroll'
					const hasHorizontalScroll = cur.scrollWidth - cur.clientWidth > 4
					const hasVerticalScroll = cur.scrollHeight - cur.clientHeight > 4
					if ((scrollableX && hasHorizontalScroll) || (scrollableY && hasVerticalScroll)) {
						return cur
					}
				}
	...
	function isTextNodeVisibleInNearestScrollable(textNode) {
		const parentElement = textNode.parentElement
		if (!parentElement) return true
		const scroller = getNearestScrollableAncestor(parentElement)
	...
	function isElementAccepted(element) {
		...
		if (!leafElementDenyList.has(tagName)) return true

		// SVGs used as interactive controls (star ratings, icon buttons, etc.) carry
		// click handlers but no HTML attributes — cursor:pointer is the reliable signal.
		// Also allow explicit role/tabindex/onclick as secondary checks.
		if (tagName === 'svg') {
			try {
	...
	function isElementVisible(element) {
		const style = getCachedComputedStyle(element)
		// offsetWidth/offsetHeight are undefined for SVG elements; fall back to getBoundingClientRect.
		const hasOffsetSize = element.offsetWidth > 0 && element.offsetHeight > 0
		let hasBBoxSize = false
			if (!hasOffsetSize) {
				try {
					const r = element.getBoundingClientRect()
					hasBBoxSize = r.width > 0 && r.height > 0
				} catch (_) {}
			}
			if (!(
				(hasOffsetSize || hasBBoxSize) &&
				style?.visibility !== 'hidden' &&
				style?.display !== 'none'
			)) return false
			const rect = getCachedBoundingRect(element)
			return hasVisibleRectAfterAncestorClipping(rect ? [rect] : [], element)
		}

	function isHiddenByOpacityOnly(element) {
		const style = getCachedComputedStyle(element)
		if (!(
		...
		while (el && el !== document.documentElement) {
			const s = getCachedComputedStyle(el)
			if (s && s.opacity === '0') return true
			el = el.parentElement
		}
		return false
	}

	function cssStyleRevealsElement(style) {
		...
		if (visibility && visibility !== 'hidden' && visibility !== 'collapse') return true
		const opacity = style.opacity || style.getPropertyValue?.('opacity')
		if (opacity) {
			const opacityValue = Number.parseFloat(opacity)
			if (!Number.isNaN(opacityValue) && opacityValue > 0) return true
		}
		return false
	}

	function addHoverRevealRulesFromList(ruleList, out) {
		if (!ruleList) return
		for (const rule of Array.from(ruleList)) {
			try {
				if (rule.selectorText && rule.selectorText.includes(':hover') && cssStyleRevealsElement(rule.style)) {
					for (const rawSelector of rule.selectorText.split(',')) {
						const selector = rawSelector.trim()
						const hoverIndex = selector.indexOf(':hover')
						if (hoverIndex < 0) continue
						const parentSelector = selector.slice(0, hoverIndex).trim()
						const tail = selector.slice(hoverIndex + ':hover'.length).trim()
						if (!parentSelector || !tail) continue
						if (tail.startsWith(':') || tail.startsWith('+') || tail.startsWith('~')) continue
						const targetSelector = tail.startsWith('>') ? `:scope ${tail}` : tail
						if (targetSelector.includes(':hover')) continue
						out.push({ parentSelector, targetSelector })
					}
				}
				if (rule.cssRules) addHoverRevealRulesFromList(rule.cssRules, out)
			} catch (_) {}
		}
	}

	function getHoverRevealRules() {
		if (HOVER_REVEAL_RULES !== null) return HOVER_REVEAL_RULES
		const rules = []
		for (const sheet of Array.from(document.styleSheets || [])) {
			try {
				addHoverRevealRulesFromList(sheet.cssRules, rules)
			} catch (_) {}
		}
		HOVER_REVEAL_RULES = rules
		return rules
	}

	function isHiddenForHoverReveal(element) {
	...
	function isActionableHoverRevealTarget(element) {
		if (!element || element.nodeType !== Node.ELEMENT_NODE) return false
		if (element.inert || element.getAttribute('aria-hidden') === 'true') return false
		if (element.disabled) return false

		const tagName = getTagName(element)
		if (new Set(['a', 'button', 'input', 'select', 'textarea', 'details', 'summary', 'label']).has(tagName)) {
			return true
		}

		const role = element.getAttribute('role') || element.getAttribute('aria-role')
		const hoverRevealRoles = new Set([
			'button',
			'link',
			'menuitem',
			'menuitemradio',
		...
		if (role && hoverRevealRoles.has(role)) return true
		...
		if (element.__trackedEvents && element.__trackedEvents.size > 0) return true

		try {
			const style = getCachedComputedStyle(element)
			if (style?.cursor === 'pointer' || style?.cursor === 'context-menu') return true
		} catch (_) {}

		return false
	}

	function hasHoverRevealActionableDescendant(element) {
		if (!element || element.nodeType !== Node.ELEMENT_NODE) return false
		if (!isElementVisible(element)) return false
		if (element.getAttribute('aria-hidden') === 'true' || element.inert) return false

		const tagName = getTagName(element)
		if (new Set(['html', 'body', 'main', 'article', 'section', 'nav', 'header', 'footer']).has(tagName)) {
			return false
		}

		for (const rule of getHoverRevealRules()) {
			try {
				if (!element.matches(rule.parentSelector)) continue
				const targets = Array.from(element.querySelectorAll(rule.targetSelector))
				for (const target of targets) {
					if (target === element || !element.contains(target)) continue
					if (!isHiddenForHoverReveal(target)) continue
					if (isActionableHoverRevealTarget(target)) return true
				}
			} catch (_) {}
		}
		return false
	...
	function isInteractiveElement(element) {
		...
		function doesElementHaveInteractivePointer(element) {
			if (getTagName(element) === 'html') return false
		...
		if (hasHoverRevealActionableDescendant(element)) return true

		// check whether element has event listeners by window.getEventListeners
		try {
			if (typeof getEventListeners === 'function') {
				const listeners = getEventListeners(element)
				const mouseEvents = ['click', 'mousedown', 'mouseup', 'dblclick', 'contextmenu']
			...
			const commonMouseAttrs = ['onclick', 'onmousedown', 'onmouseup', 'ondblclick', 'oncontextmenu']
		...
		// Check for tracked addEventListener calls (injected by _EVENT_TRACKER_SCRIPT)
		if (element.__trackedEvents && element.__trackedEvents.size > 0) {
			return true
		}
	...
	function isInteractiveCandidate(element) {
		...
		const hasAriaAttribute = (element.getAttributeNames?.() || []).some((name) =>
	...
	function isElementDistinctInteraction(element) {
		...
		if (hasHoverRevealActionableDescendant(element)) {
			return true
		}
   ...
   */
		...
		if (shouldHighlight) {
			...
			if (nodeData.isInViewport || viewportExpansion === -1) {
				...
				try {
					...
					nodeData.boundingBox = { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) }
				} catch (_) {}
	...
	function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
		...
		if (node.nodeType === Node.TEXT_NODE) {
			...
			if (!parentElement || getTagName(parentElement) === 'script') {
				return null
			}
			...
			DOM_HASH_MAP[id] = {
				...
				isVisibleInScrollable: isTextNodeVisibleInNearestScrollable(node),
			}
			return id
		}
     ...
     */
		...
		if (
			...
			getTagName(node) === 'iframe' ||
			getTagName(node) === 'body'
		) {
			...
			/**
			 * @edit capture live DOM value properties for form elements.
			 * HTML attributes reflect initial state; DOM properties reflect current state.
			 */
			const _tag = getTagName(node)
			if (_tag === 'select' || _tag === 'textarea') {
				if (node.value != null) nodeData.attributes['value'] = node.value
			} else if (_tag === 'input') {
				const _inputType = (node.getAttribute('type') || 'text').toLowerCase()
				if (_inputType !== 'checkbox' && _inputType !== 'radio' &&
					_inputType !== 'file' && _inputType !== 'button' &&
					_inputType !== 'submit' && _inputType !== 'reset') {
					if (node.value != null) nodeData.attributes['value'] = node.value
				}
			}
		}
		...
		if (node.nodeType === Node.ELEMENT_NODE) {
			...
			} else if (isHiddenByOpacityOnly(node) && isInteractiveElement(node)) {
